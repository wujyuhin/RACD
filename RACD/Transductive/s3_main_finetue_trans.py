...
def parse_args(data_name):
    ...
    parser.add_argument('--times', help='the number of runs', default=5)
    ...
    parser.add_argument('--save_path', help='the save path of all results',
                        default=f'./RACD/Transductive/report/RACD_report/')
    parser.add_argument('--data', help='the name of dataset', default=f'{data_name}')
    parser.add_argument('--model_name', help='the name of model',
                        default=f'/finetune_mdoel/')
    parser.add_argument('--pretrain_model_path', help='the path of the weights of pertained model',
                        default=f'./RACD/Transductive/report/EAD_report/{data_name}/pretrain_model/{opt}/')  
    parser.add_argument('--train_file', help='the path of the train file',
                        default=f'./data/{data_name}/TransData/train.csv')
    parser.add_argument('--valid_file', help='the path of the valid file',default=f'./data/{data_name}/TransData/val.csv')
    parser.add_argument('--test_file', help='the path of the test file', default=f'./data/{data_name}/TransData/test.csv')
    parser.add_argument('--Q_matrix', help='the path of the q-matrix', default=f'./data/{data_name}/TransData/Q_mat.npy')
    ...
    parser.add_argument('--epoch', help='the training epoch', default=40) 
    parser.add_argument('--change_epoch', help='the epoch of changing unsupervised to supervise', default=30)
    ...
    parser.add_argument('--k', help='the number of samples retrieved from hashing used for aggregation',default=20)
    parser.add_argument('--T', help='the temperature in the contrasive learning', default=0.5)
    parser.add_argument('--weight', help='the weight of the contrasive loss', default=1)
...
def train(model, optimizer_hash, optimizer_decoder, trainloader, trainset, validloader, df_valid,
          times, weight, change_epoch, testloader, df_test, early_stopping=None):
    since = time.time()
    best_acc = -1
    bestEpoch = 0
    ...
    for epoch in range(change_epoch):
        ...
        msg = "epoch:{} train_acc:{:.4f} Lcontra:{:.4f} || val_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f} || test_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f}\n".format(
            epoch + 1,
    ...
    for epoch_supervise in range(change_epoch, args.epoch + epoch):
        # trainset.set_epoch(epoch)
        result_per_epoch = train_one_epoch(model, optimizer_hash, optimizer_decoder, trainloader, trainset,
        ...
        msg = "epoch:{} train_acc:{:.4f} Lce:{:.4f}  ||  val_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f} ||  test_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f} \n".format(
            epoch_supervise + 1,
            result_per_epoch['train_acc'],
...
if __name__ == '__main__':
    ...
    n_user = np.max(df_train['user_id']) + 1
    n_item = np.max([np.max(df_train['item_id']), np.max(df_valid['item_id']), np.max(df_test['item_id'])]) + 1
    n_know = Q_mat.shape[1]

    df_train = add_knowledge_code(df_train, Q_mat)
    df_valid = add_knowledge_code(df_valid, Q_mat)

    trainset = RCDataset(df_train, n_user, n_item)
    trainloader = DataLoader(dataset=trainset, batch_size=batch_size, shuffle=True)

    validset = RCDataset(df_valid, n_user, n_item)
    validset.log_mat = trainset.log_mat
    ...
    testset = RCDataset(df_test, n_user, n_item)
    testset.log_mat = trainset.log_mat
    ...
    for i in range(times):
        ...
        train(model, optimizer_hash, optimizer_decoder, trainloader, trainset, validloader, df_valid, i, weight,
              change_epoch, testloader, df_test, early_stopping=early_stopping)
        model.load_state_dict(torch.load(os.path.join(exp_path, "best.pth"),
                                         weights_only=False)['model'])
        ...
        test_result = eval(model, testloader, df_test, test=True)
        ...
        record_test = "Run {} best_model test_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f} \n".format(
            i + 1,
    ...
    record_test_average = 'Summarize: \n test_acc = %.4f ± %.4f \n test_auc = %.4f ± %.4f \n test_rmse = %.4f ± %.4f \n test_f1 = %.4f ± %.4f' % (
        np.mean(test_acc_all) * 100,
        np.std(test_acc_all) * 100,
        np.mean(test_auc_all) * 100,
        np.std(test_auc_all) * 100,
        np.mean(test_rmse_all) * 100, 
        np.std(test_rmse_all) * 100,
        np.mean(test_f1_all) * 100,
        np.std(test_f1_all) * 100)
    print(record_test_average)
    with open(os.path.join(out_path, "test_performance.txt"), 'a') as f:
        f.write('\n')
        f.write("train_file:{} \n valid_file:{} \n test_file:{} \n".format(args.train_file, args.valid_file,
                                                                           args.test_file))
        f.write(record_test_average)
        f.write('\n')
