...
from dataset import DistillDataset, RCDataset
import argparse
...
def parse_args(data_name):
    ...
    parser.add_argument('--train_file', help='the path of the train file',default=f'./data/{data_name}/TransData/train.csv')
    parser.add_argument('--theta_file', help='the path of the NCDM_theta file',default=f'./data/{data_name}/TransData/NCDM_theta.csv')
    parser.add_argument('--psi_file', help='the path of the NCDM_psi file',default=f'./data/{data_name}/TransData/NCDM_psi.csv')
    ...
    parser.add_argument('--save_path', help='the save path of all results',
                        default=f'./RACD/Transductive/report/EAD_report/')
    parser.add_argument('--model_name', help='the name of model',default=f'/pretrain_model/')
...
if __name__ == '__main__':
    ...
    trainset = DistillDataset(df_train, n_user, n_item, df_theta, df_psi)
    ...
    for i in range(times):
        ...
        train(model, optimizer, trainloader, trainset, validloader, df_valid, exp_path, epoch, alpha)
        # 导入模型进行测试
    ...
    record_test_average = 'ALL_Run test_acc = %.4f ± %.4f \n test_auc = %.4f ± %.4f \n test_rmse = %.4f ± %.4f \n test_f1 = %.4f ± %.4f' % (
