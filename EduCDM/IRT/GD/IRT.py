...
class IRTNet(nn.Module):
    ...
    def forward(self, user, item):
        ...
        if torch.max(theta != theta) or torch.max(a != a) or torch.max(b != b):  # pragma: no cover
            # 源代码此处报错，因为张量无穷大，将theta,a,b中的nan值替换为0
            theta[theta != theta] = 0
            a[a != a] = 0
            b[b != b] = 0
    ...
    def train(self, train_data, test_data=None, *, epoch: int, device="cpu", lr=0.001,save_dir) -> ...:
        ...
        for e in range(epoch):
            ...
            for batch_data in tqdm(train_data, "Epoch %s   " % e):
                ...
                # if (e >= 61) and (batch_num >= 85):
                #         print("epoch:{} batch_num:{}".format(e, batch_num))
            ...
            #     accuracy,auc,rmse = self.eval(test_data, device=device)
            #     if accuracy > bestAccuracy:
            #         self.save(os.path.join(save_dir,"best_irt.pth"))
            #     print("[Epoch %d] accuracy: %.6f, auc: %.6f, rmse: %.6f" % (e, accuracy,auc, rmse))
