...
class IDCDataset(Dataset):
    ...
    def __init__(self, df_log: pd.DataFrame, n_user:int, n_item:int, Q_mat = None):
        ...
        pbar = tqdm(total = df_log.shape[0],desc='Loading data_triple')
    ...
    def __getitem__(self, index):
        ...
        # # 计算user_log_mask里面非零个数
        rate = 0.2
        # 步骤1: 找出所有非零元素的位置
        non_zero_indices = np.where(user_log_mask != 0)[0]
        # 步骤2: 计算需要置零的数量
        num_to_zero = int(len(non_zero_indices) * rate)
        # 步骤3: 随机选择位置进行置零
        if num_to_zero > 0:
            zero_indices = np.random.choice(non_zero_indices, size=num_to_zero, replace=False)
            user_log_mask[zero_indices] = 0
        return user_log_mask, item_log_mask, user_id, item_id, score
