            ...
            if test_data is not None:
                ...
                if accuracy > bestAccuracy:
                    ...
                    if save_dir is not None:
                        ...
                        self.save("best_mirt.pth")
                print("[Epoch %d] train_acc: %.6f, val_acc: %.6f, auc: %.6f, rmse: %.6f, f1: %.6f" % (e, accuracy, accuracy, auc, rmse,f1))

            msg = "epoch:{} train_acc:{:.4f} val_acc:{:.4f} auc:{:.4f} rmse:{:.4f} f1:{:.4f} \n".format(
                e + 1, accuracy_train, accuracy, auc, rmse,f1)
            f.write(msg)
            f.flush()
        msg_best = "model:{} best epoch:{} val_acc:{:.4f} \n".format('MIRT', bestEpoch, bestAccuracy)
